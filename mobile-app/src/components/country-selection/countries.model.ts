...
export class Countries {
    countryStringForCode(code: string): string {
        let obj = CountryList.filter((element, index, array) => {
            return element.code == code;
        });
      
        // obj[0]['emoji'] + "&nbsp; " + 
        return  obj[0]['emoji'] + " " + obj[0]['name'];
    }



    filter(country: string): any[] {
        return CountryList.filter((element, index, array) => {
            return element.name.toLowerCase().indexOf(country.toLowerCase()) > -1;
        });
    }
}
